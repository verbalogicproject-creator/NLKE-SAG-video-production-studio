...
export const LAB_MCP_TOOLS: McpTool[] = [
  ...
  { name: 'computer.activities.list', description: 'List workspace-scoped active-tab computer-use activities and their expiry/state.', requiredScope: 'drafts:read', inputSchema: object({}) },
  { name: 'computer.activity.get', description: 'Read one bounded computer-use activity and its pinned signed profile.', requiredScope: 'drafts:read', inputSchema: object({ activityId: { type: 'string' } }, ['activityId']) },
  { name: 'computer.actions.list', description: 'List only signed-profile actions currently available to an active tab; unprofiled tabs return none.', requiredScope: 'drafts:read', inputSchema: object({ activityId: { type: 'string' } }, ['activityId']) },
  { name: 'computer.intent.create', description: 'Create a one-use intent for a visible profile-eligible read or safe reversible action.', requiredScope: 'edits:write', inputSchema: object({ activityId: { type: 'string' }, requestId: { type: 'string' }, beforeObservationId: { type: 'string' }, actionId: { type: 'string' }, targetBindingId: { type: 'string' }, arguments: { type: 'object' }, contextRef: { type: 'object' }, expectedProjectRevision: { type: 'integer', minimum: 1 } }, ['activityId', 'requestId', 'beforeObservationId', 'actionId', 'targetBindingId', 'arguments']) },
  { name: 'computer.intent.execute', description: 'Consume a one-use computer-use execution ticket. Effect success still requires an after observation from the extension.', requiredScope: 'edits:write', inputSchema: object({ intentId: { type: 'string' }, ticket: { type: 'string' } }, ['intentId', 'ticket']) },
  { name: 'computer.receipt.get', description: 'Read an append-only browser effect receipt and its explicit same-adapter verification limitation.', requiredScope: 'evidence:read', inputSchema: object({ receiptId: { type: 'string' } }, ['receiptId']) },
