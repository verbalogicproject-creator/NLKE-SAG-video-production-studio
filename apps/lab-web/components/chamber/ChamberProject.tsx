...
function VariantEditor({ base, variant, invoke, busy }: { base: string; variant: Variant; invoke: (path: string, init?: RequestInit) => Promise<any>; busy: string }) {
  ...
  async function editTitle(itemId: string, previous: string) {
    const text = window.prompt('Hook title', previous); if (!text || !project) return;
    const body = await invoke(`${base}/engine-project`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ command: 'timeline.set_title', arguments: { item_id: itemId, text }, expectedRevision: project.revision }) });
  ...
  return <div className="space-y-2">
    ...
    {titles.map((title: any) => <button key={title.id} onClick={() => editTitle(title.id, title.text)} className="w-full text-left border border-border-base p-2 text-xs">HOOK · {title.text}</button>)}
