...
def set_selection(item_ids: list[str], expected_revision: int, project_id: str = DEFAULT_PROJECT_ID) -> dict[str, Any]:
    """Project stable timeline identities into the shared browser/agent context; selection grants no authority."""
    with _client() as client:
        return _json(
            client.post(
...
@mcp.tool()
def move_timeline_item(item_id: str, start_ticks: int, expected_revision: int, project_id: str = DEFAULT_PROJECT_ID) -> dict[str, Any]:
    """Move a stable timeline item to an exact integer-tick position."""
    return _semantic_command(project_id, "timeline.move_item", {"item_id": item_id, "start_ticks": start_ticks}, expected_revision)
...
def delete_timeline_item(item_id: str, expected_revision: int, project_id: str = DEFAULT_PROJECT_ID) -> dict[str, Any]:
    """Delete a timeline item; project.undo remains a compensating operation."""
